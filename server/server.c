...
static SERVICE g_srv[] = {
	// 存放六个进程路径的数组。第一个为路径，第二个为进程号，当进程号（返回值）为-1时，说明进程未启动
	{"./open", -1},		// 开户
	{"./close", -1},	// 销户
	{"./save", -1},		// 存款
	{"./withdraw", -1}, // 取款
...
int start(void) // 运行六个业务的进程
{
	...
	for (i = 0; i < sizeof(g_srv) / sizeof(g_srv[0]); i++) // 总数据类型的大小除以某一成员数据类型的大小，就是数组成员的个数，为6。循环6次，打开六个业务进程
	{
		...
		// vfork如果创建子进程失败，则返回-1,然后赋值给要打开进程的进程号，说明该进程打开失败。
		// vfork()与普通的fork()不同，它创建的子进程与父进程共享内存空间。创建成功，在父进程中返回子进程ID，在子进程中返回0.
		{
			perror("vfork");
			return -1;
		}
...
int main(void)
{
	...
	// 正常退出：用户按Enter → 调用stop() → return 0 → 自动调用deinit()
	// 强制退出：用户按Ctrl+C → 触发sigint() → 调用stop() → exit(0) → 自动调用deinit()
}
